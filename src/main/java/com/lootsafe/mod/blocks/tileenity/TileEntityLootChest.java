...
public class TileEntityLootChest extends TileEntityLockableLoot implements ITickable {
	...
	{     		
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerLootChest)
                {
                    if (((ContainerLootChest)entityplayer.openContainer).getChestInventory() == this)
                    {
